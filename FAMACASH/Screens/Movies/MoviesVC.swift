...
extension MoviesVC {
    ...
    private func loadMovies(movieType: MovieType){
        ...
        MovieService.init().getMovies(movieType: movieType, pageIndex: "\(pageIndex)", language: currentLanguage) { [weak self] response in
            guard let self = self else { return }
            switch response {
            case .success (let movies):
                self.movieViewModelList.append(contentsOf: movies.map( { return MoviesViewModel($0)}))
...
//            self.present(selectedMovieVC, animated: true, completion: nil)
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
         if (indexPath.row == movieViewModelList.count - 2 ) {
            self.pageIndex = self.pageIndex + 1
            self.loadMovies(movieType: movieType)
         }
    }
